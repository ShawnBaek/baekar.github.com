											  ...
											  std::vector<int> indexChange)
{
	...
	//모든 스케일에 대한 커널 크기 값을 정해둔다, 각 point별 커널 크기는 스케일에 비례
...
__inline__ int BriskDescriptorExtractor::smoothedIntensity(const cv::Mat& image,
   ...
   //patternPoints [scale][rot][index] 3차원 패턴 --> 1차원
...
void BriskDescriptorExtractor::computeImpl(const Mat& image, 
	...
	//scale 결정 후 border를 넘어가는 keypoint를 삭제하고 남은 keypoint를 사용한다
	if(!scaleInvariance)
	...
	for(size_t k=0; k<ksize; k++){
	   ...
	   const int& scale=keypoints[k].size;
...
{
	BriskScaleSpace briskScaleSpace(this->octaves);
