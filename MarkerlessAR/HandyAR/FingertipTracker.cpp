...
/*
			...
			if ( _nFingertips == 5 )
            {
                _fDetected = true;
                break;
            }else if (_nFingertips==4){
			}else if ( _nFingertips == 3){
				printf("33333333333333333333333333333333Finger 3\n");
				SortFingerTip();
				MOVE_COUNTER++;
				NOTHING_COUNTER=0;
				if(NOTHING_COUNTER==0 && MOVE_COUNTER==5){
					SCALE_COUNTER=0;
					SCALE_ACTION_START_DISTANCE=0.0f;
					SCALE_ACTION_DISTANCE=0.0f;
					MOVE_ACTION_START_POINT.x=_fingertipPoints[2].x;//thumb
					MOVE_ACTION_START_POINT.y=_fingertipPoints[2].y;//thumb
					printf("move start point: %f %f\n",MOVE_ACTION_START_POINT.x,MOVE_ACTION_START_POINT.y);
				}else if(MOVE_COUNTER>5){
					if(MOVE_COUNTER==6||_fingertipPoints[2].x<=MOVE_ACTION_POINT.x+5 && _fingertipPoints[2].x>=MOVE_ACTION_POINT.x-5 && _fingertipPoints[2].y<=MOVE_ACTION_POINT.y+5 && _fingertipPoints[2].y>=MOVE_ACTION_POINT.y-5){
						MOVE_ACTION_POINT.x=_fingertipPoints[2].x-MOVE_ACTION_START_POINT.x;
						MOVE_ACTION_POINT.y=_fingertipPoints[2].y-MOVE_ACTION_START_POINT.y;
						printf("moving difference: %f %f\n",MOVE_ACTION_POINT.x,MOVE_ACTION_POINT.y);
					}else{
						printf("moving difference: TOO BIG!!\n");
					}
					
				}
				printf("33333333333333333333333333333333Finger 3\n");
			}else if(_nFingertips == 2){
				printf("22222222222222222222222222222222Finger 2\n");
				SortFingerTip();
				SCALE_COUNTER++;
				NOTHING_COUNTER=0;
				if(NOTHING_COUNTER==0 && SCALE_COUNTER == 5){
					MOVE_COUNTER=0;
					MOVE_ACTION_START_POINT.x=0.0f;
					MOVE_ACTION_START_POINT.y=0.0f;
					MOVE_ACTION_POINT.x=0.0f;
					MOVE_ACTION_POINT.y=0.0f;
					SCALE_ACTION_START_DISTANCE = sqrt(pow(_fingertipPoints[0].x - _fingertipPoints[1].x,2)+pow(_fingertipPoints[0].y - _fingertipPoints[1].y,2));
					printf("start 2 finger:%f\n",SCALE_ACTION_START_DISTANCE);
				}else if(MOVE_COUNTER>5){

					float newSAD = sqrt(pow(_fingertipPoints[0].x - _fingertipPoints[1].x,2)+pow(_fingertipPoints[0].y - _fingertipPoints[1].y,2))-SCALE_ACTION_START_DISTANCE;
					if(MOVE_COUNTER==6 || newSAD<= SCALE_ACTION_DISTANCE+5 && newSAD>= SCALE_ACTION_DISTANCE-5){
						SCALE_ACTION_DISTANCE = newSAD;
						printf("distance difference: %f\n",SCALE_ACTION_DISTANCE);
					}else{
						printf("distance difference: TOO BIG!!\n");
					}
				}
				printf("22222222222222222222222222222222Finger 2\n");
			}else{
    ...
    //
    // When 5 fingers tips are detected, assign the fingertip index number
    //
    if ( _fDetected )
    {
		//printf("here-----------------------------------------------------------------------------\n");
...
void FingertipTracker::SortFingerTip(){
	//
	// When 3 fingers tips are detected, assign the fingertip index number
	//
