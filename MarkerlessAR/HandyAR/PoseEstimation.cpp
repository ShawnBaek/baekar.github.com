...
//    CvPoint2D32f chess_pt[NUM_CHESS_POINT];
    int chess_count = 0;
    bool fChessboardFound = cvFindChessboardCorners(
        grayImage,
        cvSize(NUM_CHESS_COL, NUM_CHESS_ROW),
        chess_pt,
        &chess_count );
    
    if ( fChessboardFound )
    {
        cvFindCornerSubPix( grayImage, chess_pt, chess_count, cvSize(10, 10), cvSize(-1, -1),
            cvTermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.02) );
        cvDrawChessboardCorners(colorImage, cvSize(NUM_CHESS_COL, NUM_CHESS_ROW),
            chess_pt, chess_count, fChessboardFound );
    }
    ...
    //
    if ( cvFindExtrinsicCameraParams3(
            &obj_pt,
            &img_pt,
            &intrinsic,
            &distortion,
...
float PoseEstimation::ComputeReprojectionError(
//    IplImage * grayImage,
    IplImage * colorImage,
    CvMat * rotation_vec,
