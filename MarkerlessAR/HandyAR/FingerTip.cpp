...
void FingerTip::FloodFill( IplImage * src, IplImage * dst, CvPoint point )
{
    ...
    for ( int i = 0 ; i < 8 ; i ++ )
    {
        ...
        if ( nextPoint.x < 0 || nextPoint.x >= src->width ||
             nextPoint.y < 0 || nextPoint.y >= src->height )
             continue;

        if ( cvGetReal2D( src, nextPoint.y, nextPoint.x ) >  0 &&
             cvGetReal2D( dst, nextPoint.y, nextPoint.x ) == 0 )
        {
...
int FingerTip::FindFingerTipCandidates( IplImage * handRegion )
{
    ...
    for ( int i = 0 ; i < _numCirclePoint ; i ++ )
    {
        ...
        if ( _circlePoint[i].x < _circleDist[ i ] * FINGERTIP_CONDITION_RANGE ||
             _circlePoint[i].y < _circleDist[ i ] * FINGERTIP_CONDITION_RANGE ||
             _pDistImage->width  - _circlePoint[i].x <= _circleDist[ i ] * FINGERTIP_CONDITION_RANGE ||
             _pDistImage->height - _circlePoint[i].y <= _circleDist[ i ] * FINGERTIP_CONDITION_RANGE )
            continue;

        // check finger tip condition
        if ( !CheckFingerTipCondition(
                _pHandImage,
                _circlePoint[i].x,
                _circlePoint[i].y,
                _circleDist[i] * FINGERTIP_CONDITION_RANGE ) )
            continue;/**/
...
{
    float distToCenter[ MAX_END_POINT ];
    for ( int i = 0 ; i < _numEndPoint ; i ++ )
        distToCenter[i] = sqrt(
            pow((float)_endPoint[i].x-_maxDistPoint.x, 2) +
            pow((float)_endPoint[i].y-_maxDistPoint.y, 2) );

    for ( int i = 0 ; i < _numEndPoint ; i ++ )
    {
        for ( int j = i+1 ; j < _numEndPoint ; j ++ )
        {
//            if ( distToCenter[i] > distToCenter[j] )
            if ( _endPointNumAdj[i] < _endPointNumAdj[j] )
            {
                ...
                //float tempDistToCenter = distToCenter[i];
                //distToCenter[i] = distToCenter[j];
                //distToCenter[j] = tempDistToCenter;
...
void FingerTip::RemoveAdjacentEndPoints()
{
    ...
    for ( int i = 0 ; i < _numEndPoint ; i ++ )
    {
        ...
        {
            float tempDist = sqrt(
                pow((float)_endPoint[i].x-_endPoint[j].x, 2) +
                pow((float)_endPoint[i].y-_endPoint[j].y, 2) );
            ...
            if ( _endPointDist[j] > tempDist )
            {
                ...
                //
                float distToMaxI = sqrt(
                    pow((float)_endPoint[i].x-_maxDistPoint.x, 2) +
                    pow((float)_endPoint[i].y-_maxDistPoint.y, 2) );
                float distToMaxJ = sqrt(
                    pow((float)_endPoint[j].x-_maxDistPoint.x, 2) +
                    pow((float)_endPoint[j].y-_maxDistPoint.y, 2) );
...
void FingerTip::AddFingerTip( int index )
{
    ...
    {
        float tempDist = sqrt(
            pow((float)_circlePoint[i].x-_circlePoint[index].x, 2) +
            pow((float)_circlePoint[i].y-_circlePoint[index].y, 2) );
        ...
        if ( _circleDist[index] > tempDist )
        {
            ...
            //
            float distToMaxI = sqrt(
                pow((float)_circlePoint[i].x-_maxDistPoint.x, 2) +
                pow((float)_circlePoint[i].y-_maxDistPoint.y, 2) );
            float distToMaxIndex = sqrt(
                pow((float)_circlePoint[index].x-_maxDistPoint.x, 2) +
                pow((float)_circlePoint[index].y-_maxDistPoint.y, 2) );
...
void FingerTip::FindSecondRankPoints()
{
    ...
    if ( i1 < 0 ) i1 = 0;
    if ( i2 > _pDistImage->height ) i2 = _pDistImage->height;
    if ( j1 < 0 ) j1 = 0;
    if ( j2 > _pDistImage->width ) j2 = _pDistImage->width;
    ...
    for ( int i = i1 ; i < i2 ; i ++ )
    {
        for ( int j = j1 ; j < j2 ; j ++ )
        {
            ...
            if ( GetRankInNeighbor( j, i ) == 2 )
            {
                ...
                if ( Radius > 3 && Radius <= _maxDistValue * 0.3 )//30 )
                {
...
int FingerTip::GetRankInNeighbor( int x, int y )
{
    ...
    for ( int i = -1 ; i <= 1 ; i ++ )
    {
        for ( int j = -1 ; j <= 1 ; j ++ )
        {
            if ( i+x < 0 || i+x >= _pDistImage->width ||
                 j+y < 0 || j+y >= _pDistImage->height )
                continue;
...
void FingerTip::FindEndPoints( int index, int prevIndex, int prevIndex2 )
{
    // check image boundary
    if ( !(
         _circlePoint[index].x < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE ||
         _circlePoint[index].y < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE ||
         _pDistImage->width  - _circlePoint[index].x < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE ||
         _pDistImage->height - _circlePoint[index].y < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE )
         )
    {
        // check fingertip condition
        if ( CheckFingerTipCondition(
                _pDistImage,
                _circlePoint[index].x,
                _circlePoint[index].y,
                _circleDist [index] * FINGERTIP_CONDITION_RANGE )
            )
        {
...
//            tempDist -= _circleDist[i];
            if ( _circleDist[index] + _circleDist[i] > tempDist &&
                 tempDist + _circleDist[i] < minDist )
            {
    ...
    if ( 0 )//fEndpoint )
    {
        // no more point to visit

        // check the direction

        // check image boundary
        if ( _circlePoint[index].x < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE ||
             _circlePoint[index].y < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE ||
             _pDistImage->width  - _circlePoint[index].x < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE ||
             _pDistImage->height - _circlePoint[index].y < _circleDist[ index ] * FINGERTIP_CONDITION_RANGE )
            return;
...
int FingerTip::FindFingerTipCandidatesByTemplateMatching( IplImage * handRegion )
{
    ...
    for ( int i = 0 ; i < _circleMatch5->height ; i ++ )
    {
        for ( int j = 0 ; j < _circleMatch5->width ; j ++ )
        {
            ...
            if ( //cvGetReal2D( handRegion, i+22, j+22 ) > 0 &&
                 //cvGetReal2D( fingerTip, i+22, j+22 ) < match &&
                 //cvGetReal2D( dist, i+22, j+22 ) >= 5 &&
                 //image->imageData[ (i+22) * image->widthStep + (j+22) * 3 + 0 ] == 255 && 
                 CheckFingerTipCondition( handRegion, j+nMidPos, i+nMidPos, nRadius * FINGERTIP_CONDITION_RANGE ) &&
                 match >= 0.7 )
            {
...
bool FingerTip::CheckFingerTipCondition( IplImage * image, int x, int y, int size )
{
    ...
    for ( int i = -1 ; i <= 1 ; i ++ )
    {
        for ( int j = -1 ; j <= 1 ; j ++ )
        {
            if ( i == 0 && j == 0 )
                continue;
            if ( i*size+x < 0 || i*size+x >= image->width ||
                 j*size+y < 0 || j*size+y >= image->height )
                continue;

            if ( cvGetReal2D( image, j*size+y, i*size+x ) > 2 ) // <<-- 0 or 1 or 2 or ??
            {
                numHandRegion ++;
                if ( numHandRegion > 2 ) return false;
                if ( numHandRegion == 2 )
                {
    ...
    if ( numHandRegion == 0 ) return false;
...
int FingerTip::FindFingerTipCandidatesByCurvature( IplImage * handRegion, bool fScreenshot )
{
    ...
    if ( fScreenshot )
    {
		...
		//아래의 주석처리는 내가 임의로 수정함.... 2012.03.21
    ...
    if ( _numEndPoint == 0 )
    {
        ...
        if ( _maxDistPoint.x >= _pDistImage->width - 1 ) _maxDistPoint.x --;
        if ( _maxDistPoint.y >= _pDistImage->height- 1 ) _maxDistPoint.y --;
    }
    ...
    for ( ; pContours ; pContours = pContours->h_next )
    {
        ...
        {
            cvSetZero( _pHandImage );
			//아래의 코드는 살짝 수정함... Thickness 값을 -1에서 2로 수정
            cvDrawContours( _pHandImage, pContours, cvScalar(255), cvScalar(0), 0, -1, 8 );
	...
	//여기서 다시 그려주는 Contour 정보들은 Ellipse 연산을 수행했기 때문에 위에서 계산된 값보다 크게 나온다.
    ...
    for ( int i = 0 ; i < pContours->total ; i ++ )
    {
        ...
        if ( fScreenshot && i > 0 )
        {
			//각 라인의 좌표에 x2를 해주고 있다. 이 의미는 320*240 사이즈에서 ellipse일 경우 곱해줬다는 의미..
    ...
    if ( fScreenshot )
    {
        cvLine( pScreenshot_ellipse, cvPoint(contour[pContours->total-1].x * 2, contour[pContours->total-1].y * 2),
            cvPoint(contour[0].x * 2, contour[0].y * 2), CV_RGB(255,255,255), 1, 8, 0 );
    }
    ...
    for ( int i = 0 ; i < pContours->total ; i ++ )
    {
        ...
        for ( int k = min_curvature_step ; k <= max_curvature_step ; k ++ )
        {
            ...
            // dot product ( cosine )
            float K = ( pp1_x*pp2_x + pp1_y*pp2_y )
            ...
            if ( nPointsGap >= GAP_POINT_THRESHOLD )
            {
                if ( nPointsConnected > CONNECTED_POINT_THRESHOLD )
                {
                    ...
                    if ( contour[minPointIndex].x > 10 &&
                         contour[minPointIndex].x < _pDistImage->width - 10 &&
                         contour[minPointIndex].y > 10 &&
                         contour[minPointIndex].y < _pDistImage->height - 10 )/**/
                    {
                        ...
                        if ( fScreenshot )
                        {
                            cvCircle( pScreenshot_curvature, cvPoint(meanPointX*2, meanPointY*2),
                                4, CV_RGB(255,0,0), -1, 8, 0 );
                            cvCircle( pScreenshot_ellipse, cvPoint(meanPointX*2, meanPointY*2),
                                4, CV_RGB(255,0,0), -1, 8, 0 );
                        }
