...
#define IMG_SIZE_WIDTH 512
#define IMG_SIZE_HEIGHT 384
...
public :
	...
	bool LoadFile(char *filename) ;		// Reads camera parameters from a file.
	...
	void DrawCamera(float size=30) ;	// Draw camera 
	void DrawAxis(double length) ;		// Draw the camera axis
	...
	//my code
	void setParams(CvMat* matParamIntrin, CvMat* matParamExtrin);
	void setParamsExtrinsic(CvMat*);
	void setParamsIntrinsic(CvMat*);
	void setProjectionMatrix(CvMat*, CvMat*);
	void showMatrixDoubleTypeArray(double*, int row, int col);
