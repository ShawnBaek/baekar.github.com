...
#include "BaekARCamera.h"



//show matrix
void CCamera::showMatrixDoubleTypeArray(double* mat, int row, int col)
{
	int cnt  = row * col;
...
//setting object variables with external parameters
void CCamera::setParamsExtrinsic(CvMat* matParamExtrinsic)
{
	for(int j = 0 ; j < 12 ; ++j) {
		m_fExtrinsic[j] = matParamExtrinsic->data.db[j];
	}

	return;
}
void CCamera::setParamsIntrinsic(CvMat* matParamIntrinsic)
{
	m_iImageWidth = IMG_SIZE_WIDTH;
	m_iImageHeight = IMG_SIZE_HEIGHT;

	for(int i = 0  ; i < 9 ; ++i) {
		m_fIntrinsic[i] = matParamIntrinsic->data.db[i];
	}

	//	m_fFocalLength = m_fIntrinsic[4] ;
	m_fFocalLength = m_fIntrinsic[0] ;
	m_fFov = 2 * atan(m_iImageHeight/(2*m_fFocalLength)) * 180 / CAMERA_PI ;

	return;
}
void CCamera::setProjectionMatrix(CvMat* matInt, CvMat* matExt)
{
...
//setting object variables with internal, external parameters
void CCamera::setParams(CvMat* matParamIntrinsic, CvMat* matParamExtrinsic)
{
	m_iImageWidth = IMG_SIZE_WIDTH;
	m_iImageHeight = IMG_SIZE_HEIGHT;
 ...
 *
 *
 * Draw Camera in OpenGL Space 
 */


void CCamera::DrawCamera(float size) 
{


	double trans[16] ;
	GetInvExtrinsicOGL(trans) ;
 ...
 * \param length 
 *
 * Draw the xyz axes in the camera's local coordinates 
 */
void CCamera::DrawAxis(double length)
{
...
*/
bool CCamera::LoadFile(char *filename) 
{
	char	szLine[256];
    ...
    while (fgets (szLine, 256, file) != NULL) 
	{
		...
		if(!strncmp(szLine, "Width", 5)) 
		{
			sscanf(szLine, "Width %d", &imgWidth) ;	
			continue ;
		} // if(!strncmp(szLine, "Width", 5))

		if(!strncmp(szLine, "Height", 6)) 
		{
			sscanf(szLine, "Height %d", &imgHeight) ;	
			continue ;
		} // if(!strncmp(szLine, "Width", 5))
		...
		//	szLine[1] == 'N' && 
		//	szLine[2] == 'T')	
		if(!strncmp(szLine, "INT", 3))
		{
			sscanf(szLine, "INT %lf %lf %lf %lf %lf %lf %lf %lf %lf", 
			...
			szLine[2] == 'J')	
		{
			...
			sscanf(szLine, "PRJ %f %f %f %f %f %f %f %f %f %f %f %f", 
				   &m_fProjection[0], &m_fProjection[1], &m_fProjection[2],
				   &m_fProjection[3], &m_fProjection[4], &m_fProjection[5],
				   &m_fProjection[6], &m_fProjection[7], &m_fProjection[8],
				   &m_fProjection[9], &m_fProjection[10], &m_fProjection[11]) ;
		}
